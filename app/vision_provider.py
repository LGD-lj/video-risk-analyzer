...
{{
  ...
  "long_term_risk": true,
  "long_term_reason": "施工围挡通常持续数周，属于长期固定障碍，需纳入路线长期风险库。"
}}

risk_score 评分指南：

强风险（score 80-100，优先保留）：
- 施工/修路/围挡：80-95
- 锥桶/临时导流：75-90
- 限高/低净空/桥洞/顶棚：85-100
- 闸口/门岗/隔离墩导致通道明显收窄：80-95
- 货车/工程车明显遮挡或占道：75-90
- 路面异常（坑洼/积水/碎石）：70-85
- 明显需要人工复核或低速通过：70-90

中风险（score 55-80，适量保留）：
- 明显停车占道压缩通行空间：60-80
- 非机动车混行/行人横穿：55-75
- 窄路/会车空间不足：55-75
- 出入口密集/商铺门口影响通行：55-70
- 视线遮挡影响路况判断：55-70

弱风险（score 30-55，只在明显影响通行时标记）：
- 普通非机动车路过、无混行风险：30-45
- 普通树木遮挡、不遮挡标志或路口：30-45
- 普通路边停车、未明显压缩通道：30-45
- 普通商铺门口、无车辆进出：30-40
- 短时动态车辆干扰：30-45

long_term_risk 判断标准：
标记为 true 的场景（长期存在、一周以上不会变化）：
- 修路/施工围挡/长期道路封闭
- 固定限高/低净空/桥洞/顶棚
- 固定闸口/门岗/固定隔离设施
- 长期占道设施、军事/园区门岗
标记为 false 的场景（临时或无法判断长期性）：
- 非机动车、临时停车、临时货车、行人
- 无法判断是否长期存在的停车占道
- 普通树木遮挡、商铺门口车辆

重要规则：
1. 对运营路线有影响的场景标记 has_risk=true
2. 弱风险只标记真正影响通行的，普通路过不必标记
3. description（30-80字）：画面现象 + 对通行的影响 + 是否建议人工复核
4. reason（20-50字）：为什么影响运营路线/无人车通行
5. 如果确实无风险（空旷高速），返回 has_risk=false, risk_score=0
"""
